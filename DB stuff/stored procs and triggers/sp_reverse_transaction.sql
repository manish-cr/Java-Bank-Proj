...
-- Reverses any POSTED transaction (TRANSFER, DEPOSIT, WITHDRAWAL)
-- by creating offsetting ledger entries.
-- ============================================================================
...
BEGIN
    ...
    INSERT INTO transaction (
        ...
        status, business_date, description
    ) VALUES (
        v_new_transaction_id, p_idempotency_key, v_original_txn.transaction_type,
        'PENDING', CURRENT_DATE,
        'REVERSAL: ' || p_reason || ' [Original: ' || p_original_transaction_id || ']'
    );

    -- 4. FOR EACH LEDGER ENTRY IN ORIGINAL, CREATE OPPOSITE ENTRY
    FOR v_entry IN
        SELECT le.*
    ...
    LOOP
        ...
        SELECT le2.balance_after INTO v_current_balance
        ...
        IF v_entry.entry_type = 'DEBIT' THEN
            v_new_balance := v_current_balance + v_entry.amount;
            INSERT INTO ledger_entry (
                ledger_entry_id, transaction_id, account_id,
                gl_account_code, entry_type, amount,
                balance_after, entry_date, description
            ) VALUES (
                gen_random_uuid(), v_new_transaction_id, v_entry.account_id,
                v_entry.gl_account_code, 'CREDIT', v_entry.amount,
                v_new_balance, CURRENT_DATE,
        ...
        ELSE
            ...
            INSERT INTO ledger_entry (
                ...
                gen_random_uuid(), v_new_transaction_id, v_entry.account_id,
                v_entry.gl_account_code, 'DEBIT', v_entry.amount,
                v_new_balance, CURRENT_DATE,
                'Reversal: ' || COALESCE(v_entry.description, '')
            );
        END IF;
    ...
    INSERT INTO outbox_event (
        ...
        gen_random_uuid(), v_new_transaction_id, 'TRANSACTION_REVERSED',
        jsonb_build_object(
